 ...
 */
export async function ensureOrgOwnership(
  userId: number,
  orgId: string
): Promise<void> {
...
export async function getDefaultOrgId(): Promise<string> {
  ...
  const defaultOrgBySlug = await db
    .select()
    .from(organizations)
...
export async function addXP(userId: number, orgId: string, amount: number) {
  ...
  if (rankUp) {
    console.log(
      `[BROCRAFT][EVENT] type="rank_up" userId=${userId} orgId=${orgId} oldRank="${oldRank}" newRank="${newRank}" totalXP=${newXP}`
    );
  }
...
async function hasActivityOnDate(
  userId: number,
  orgId: string,
  date: Date
): Promise<boolean> {
  ...
  const completedRecipes = await db
 ...
 */
export async function updateAndGetStreak(
  userId: number,
  orgId: string
): Promise<number> {
  const db = await getDb();
  ...
  if (hadActivityToday) {
    ...
    if (lastSignedInDay.getTime() < today.getTime()) {
      ...
      const hadActivityYesterday = await hasActivityOnDate(
        userId,
        orgId,
        yesterday
      );
      ...
      if (
        ...
        lastSignedInDay.getTime() === yesterday.getTime()
      ) {
        // Continua o streak
...
export async function completeRecipe(
  ...
  console.log(
    `[BROCRAFT][EVENT] type="recipe_completed" userId=${userId} orgId=${orgId} recipeId=${recipe.id} difficulty="${recipe.difficulty}" category="${recipe.category}" xpAwarded=${xpReward}`
  );
...
const BADGE_DEFINITIONS = {
  ...
  MESTRE_RANK: {
    ...
    criteria: (stats: UserStats) =>
      ["MESTRE_DO_MALTE", "ALQUIMISTA", "LEGEND"].includes(stats.rank),
  },
  STREAK_7: {
    type: "STREAK_7",
...
export async function checkAndAwardBadges(
  userId: number,
  orgId: string
): Promise<{ newlyAwarded: AwardedBadge[] }> {
  const db = await getDb();
  if (!db) {
    return { newlyAwarded: [] };
  }
  ...
  try {
    ...
    for (const [_key, badge] of Object.entries(BADGE_DEFINITIONS)) {
      // Pula se já tem
...
export async function createCommunityPost(
  ...
  const newPost = await db
    ...
    .from(communityPosts)
    .where(
      and(eq(communityPosts.userId, userId), eq(communityPosts.orgId, orgId))
    )
  ...
  console.log(
    `[BROCRAFT][EVENT] type="post_created" userId=${userId} orgId=${orgId} postId=${newPost[0]?.id} category="${data.category}"`
  );
